...
function reduceToFindAllResult<P extends string, N extends readonly string[]>(
  result: FindResult<P, N>
) {
...
function isNamedFind<P extends string, N extends readonly string[]>(
  result: unknown
): result is FindResult<P, N> | NotFound {
  return typeof result === "object" && !Array.isArray(result);
}

function isUnnamedFind<P extends string, N extends readonly string[]>(
  result: unknown
...
export const findAll =
  <P extends string, N extends readonly string[]>(
    pattern: P,
    ...names: N
  ): FindAllExpression<P, N> =>
  (content: string) => {
    let re: RegExp;
    try {
    ...
    let result = find<P, N>(pattern, ...names)(content) as
      | FindResult<P, N>
      | NotFound
      | false
      | string[];
