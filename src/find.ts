...
>(
  pattern: P,
  ...names: N
): FindExpression<P, N> => (content: string) => {
  let re: RegExp;
  try {
    re = new RegExp(pattern);
  } catch {
    throw new KnownError(`Invalid RegExp pattern passed into find(): ${pattern}`, "invalid-type/RegExp");
  }

  if(names && names.includes("all")) {
    throw new KnownError(`The name "all" can not be used in find() because an "all" value will always be passed back!`, "not-allowed/named-property")
  }
  ...
  if(found) {
    for(let idx = 0; idx < found.length; idx++) {
      ...
      if(names.length>0) {
        const key = idx === 0
          ? "all"
          : (idx-1) < names.length  ? names[idx-1] : `unknown_${idx}`;
        obj[key] = found[idx];
      }
    }
  }
  ...
  return (
    ...
    ? names.length > 0
      ...
      : arr
    : names.length > 0 ? { found: false } : false
